    ...
    if(op == 2){
        ...
        if(access(first,0) != 0){
            ...
            for(idx=1;idx<1000;idx++){
                ...
                sprintf(index,"%d",idx);
                strcpy(append,path);
                strncat(append,filename,ntr - filename);
                strcat(append,"(");
                strcat(append,index);
                strcat(append,")");
                strcat(append,ntr);
                if(access(append,0) != 0){
                    FILE *fp = fopen(append,"a+");
                    while (*qtr!='\n'||*(qtr+1)!='-'||*(qtr+2)!='-')
                    {
                        fprintf(fp,"%c",*qtr);
                        qtr++;
        ...
        // GET
        strcpy(buffer,"GET /success.html\0");
        buflen = strlen(buffer);
        fstr = (char *)0;
        for(i=0;extensions[i].ext!=0;i++) {
            len = strlen(extensions[i].ext);
